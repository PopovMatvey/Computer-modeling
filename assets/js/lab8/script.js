...
const SERCH_PARAMETR = 20;
const DETERM_ACCURACY_BETTA = 1.96;
const SIZE_CHOISE = 1000;
const ACCURACY = 0.5;
...
function getMathWaiting(choiseExerience, n) {
    let sumMathWaiting = 0;

    for (let i = 0; i < n; i++) {
        sumMathWaiting += choiseExerience[i];
    }

    return sumMathWaiting / n
}

// Получить дисперсию
function getDispircion(sizeChoise, mathWaiting, array) {
    let summDispertion = 0;

    for (let i = 0; i < sizeChoise; i++) {
        summDispertion += (array[i] - mathWaiting) * (array[i] - mathWaiting);
    }

    return summDispertion / sizeChoise;
}

//  Получить минимальное число выборки
function getMinSizeChoise(dispercion, determAccuracyBetta, accuracy) {
    return (determAccuracyBetta * determAccuracyBetta * dispercion / (accuracy * accuracy)) * 1000;
}

//  Получить минимальное число выборки без точности
function getMinSizeChoiseWithoutAcc() {
    return ((1.96 * 0.08) * (1.96 * 0.08)) / ((0.08 / 4) * (0.08 / 4));
}

function getMinSizeChoiseBySQO(determAccuracyBetta, accuracy, dispartion) {
    let chisl = 2 * determAccuracyBetta * determAccuracyBetta;
    let znam = (accuracy * dispartion) * (accuracy * dispartion)
    return 1 + (chisl / znam);
}
...
const MATH_WAITING = getMathWaiting(ARRAY_AWAY_FROM_START_AVERAGE, SAMPLE_SIZE);

const DISPERTION = getDispircion(SAMPLE_SIZE, MATH_WAITING, ARRAY_AWAY_FROM_START_AVERAGE);


const MIN_SIZE_SHOISE = getMinSizeChoise(DISPERTION, DETERM_ACCURACY_BETTA, ACCURACY);

const MIN_SIZE_SHOISE_WITHOUT_ACCURACY = getMinSizeChoiseWithoutAcc();

const MIN_SIZE_CHOISE_BY_SQO = getMinSizeChoiseBySQO(DETERM_ACCURACY_BETTA, ACCURACY, DISPERTION);


writeOnPage(MATH_WAITING, ".estimation-mathwaiting_value");
writeOnPage(DISPERTION, ".estimation-dispertion_value");
writeOnPage(MIN_SIZE_SHOISE, ".min-size-choise_value");
writeOnPage(MIN_SIZE_SHOISE_WITHOUT_ACCURACY, ".min-size-choise_with_acc__value");
writeOnPage(MIN_SIZE_CHOISE_BY_SQO, ".min-size-choise_by_sqo__value");

console.log(MIN_SIZE_SHOISE_WITHOUT_ACCURACY);
