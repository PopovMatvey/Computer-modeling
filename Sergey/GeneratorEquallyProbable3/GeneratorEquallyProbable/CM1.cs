...
namespace GeneratorEquallyProbable
{
  public partial class CM1 : Form
  {
    ...
    /// <summary>
    /// Последовательность плотности экспоненциального распределения
    /// </summary>
    public List<double> ExponentialProbabilityDensity = new List<double>();
    /// <summary>
    ...
    private void buttonGenerate_Click(object sender, EventArgs e)
    {
      ...
      ExponentialProbabilityDensity.Clear();
    ...
    public void CheckPirson()
    {
        ...
        case "Равновероятное распределение":
          {
            ...
            hi2 = Formuls.GetHi2Equally(ProbabilityDensity, CountNumbers, EquallyProbabilityDensity);
            NumberOfDegreesOfFreedom = Formuls.GetNumberOfDegreesOfFreedom(CountIntervals, 0);
            break;
          }
        case "Какой-то распределение":
          {
            ...
            for (double i = 0; i < 1; i += 1.0 / CountIntervals)
              ExponentialProbabilityDensity.Add(lambda * Math.Exp(-lambda * i));
            ...
            hi2 = Formuls.GetHi2Exponential(ProbabilityDensity, CountNumbers, ExponentialProbabilityDensity);
            NumberOfDegreesOfFreedom = Formuls.GetNumberOfDegreesOfFreedom(CountIntervals, 1);
            break;
          }
        default:
          break;
      }
      List<double> Hi2Zna4 = new List<double>();
      Hi2Zna4.Clear();
      Hi2Zna4.Add(0.5);
      Hi2Zna4.Add(0.5);
      Hi2Zna4.Add(0.5);
      ...
      Hi2Zna4.Add(18.61);
      Hi2Zna4.Add(19.61);
      numericUpDownHi2Tabl.Value = (decimal)Hi2Zna4[NumberOfDegreesOfFreedom - 1];

      textBoxHi2Nabl.Text = hi2.ToString();
      textBoxNumberOfDegreesOfFreedom.Text = NumberOfDegreesOfFreedom.ToString();
      checkPirson.BackColor = hi2 < (double)numericUpDownHi2Tabl.Value ? Color.GreenYellow : Color.Red;
    }
    /// <summary>
    /// Проверка по критерию Колмогорова
    /// </summary>
    ...
    public void CheckKolmogorov()
    {
        ...
        case "Какой-то распределение":
          {
            for (int i = 1; i <= CountNumbers; i++)
            {
              KolmogorovPlus.Add((double)i / CountNumbers - (1 - Math.Exp(-SequenceSort[i - 1] / Mx)));
              KolmogorovMinus.Add(1 - Math.Exp(-SequenceSort[i - 1] / Mx) - (double)(i - 1) / CountNumbers);
            }
            break;
          }
        default:
      ...
      checkKolmogorov.BackColor = alphaCalc <= alpha ? Color.GreenYellow : Color.Red;
    }
